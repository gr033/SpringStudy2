...
public class BoardController {
	...
	@GetMapping(value = {"/board/list/{pageNUM}", "/board/list/{pageNUM}/{writer}"})
	public String list(Model model, @PathVariable("pageNUM")  int pageNUM, @PathVariable(value = "writer", required = false ) String writer, HttpSession session) {	
		//로그인한 회원의 상태 유지
...
//		session.setAttribute("id", loginId);
		...
		if(writer != null && !writer.equals("")) {
			totalRecord = bs.selectWriter(writer);
			totalPage = (int)Math.ceil(totalRecord/(double)pageSIZE);
			start = (pageNUM-1)*pageSIZE+1;
			end = start+pageSIZE-1;
			model.addAttribute("list", bs.findWriter(start, end, writer));
		}else {
